...
# ───────────────── DAILY CHART ─────────────────
def fetch_daily_from_kis(token, code):
    """
    KIS inquire-daily-itemchartprice로 일봉 데이터 가져옴
    return: dict with date, code, name, close, high, low, volume, change_rate
    """
    if not token:
        return {}
    ...
    for _ in range(MAX_RETRY):
        try:
            ...
            print(f"[ERROR] fetch_daily_from_kis {code}: {e}")
            time.sleep(DELAY)
