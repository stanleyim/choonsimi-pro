"""
fetch_kis_flow.py  v5.4 (MARKET HOURS GUARD ADDED)
─────────────────────────────────────────────
KIS API → 기관/외국인 시장 수급 데이터 수집
✔ v5.3 기능 100% 유지
✔ NEW: 장중(09:00~15:45 KST) 실행 차단
✔ 오로지 장마감 자료만 사용 원칙
✔ 기존 market_flow.json 보존 (이전 영업일 자료)
─────────────────────────────────────────────
"""
...
if __name__ == "__main__":
    # ═══════════════════════════════════════════════════
    # v5.4 GUARD — 장중(09:00~15:45) 실행 차단
    # ═══════════════════════════════════════════════════
    if is_market_hours():
        now = datetime.now(KST)
        print(f"[GUARD] 장중 실행 차단 ({now:%H:%M} KST)")
        print(f"[GUARD] 기존 market_flow.json 유지 (이전 영업일 장마감 자료)")
