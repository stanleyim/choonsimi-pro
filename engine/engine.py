"""
engine.py — v7.4 MARKET HOURS GUARD ADDED
─────────────────────────────────────────────────────────
✔ v7.3.2 기능 100% 유지 (스코어링/PRO_MODE/verify 등)
✔ NEW: 장중(09:00~15:45 KST) 4중 가드
  1) enrich_with_kis() 스킵 → KIS 실시간 호출 차단
  2) verify() 스킵 → 오염 데이터 검증 방지
  3) verify_pro() 스킵 → 오염 데이터 검증 방지
  4) save_signal_history() 스킵 → 히스토리 오염 방지
✔ 오로지 장마감 자료만 사용 원칙
✔ 장중 실행 시: result.json 만 갱신 (로그 확인용)
─────────────────────────────────────────────────────────
"""
...
class StockScorer:
    ...
    def pro_filter(self, s, sc, exp):
        ...
        f = PRO_FILTER
        if sc        < f["min_score"]:      return None
        if flow      <= f["min_flow"]:      return None
        if chg       < f["chg_min"]:        return None
        if chg       > f["chg_max"]:        return None
        if vol_ratio < f["min_vol_ratio"]:  return None
        if exp       < f["min_exp_return"]: return None
...
# SIGNAL HISTORY SAVE
# ═══════════════════════════════════════════════════════
def save_signal_history(top20, entry_top5, regime, today, biz_day):
...
def run():
    ...
    # ═══════════════════════════════════════════════════
    # v7.4 GUARD — 장중 판정 (한 번만)
    # ═══════════════════════════════════════════════════
    market_open = is_market_hours()
    if market_open:
        now = datetime.now(KST)
        print(f"[GUARD] 장중 실행 감지 ({now:%H:%M} KST)")
        print(f"[GUARD] 오로지 장마감 자료만 사용 원칙")
        print(f"[GUARD] KIS 실시간 호출 / verify / signal_history 모두 스킵")

    print(f"[START] engine v7.4 Choonsimi Core  {today}  MODE={mode}")
    ...
    # ═══════════════════════════════════════════════════
    # v7.4 GUARD 1: enrich_with_kis (실시간 호출 차단)
    # ═══════════════════════════════════════════════════
    if not market_open:
        token = get_token()
        if token:
        ...
        print("[GUARD] enrich_with_kis 스킵 (KIS 실시간 호출 차단)")
    ...
    # ═══════════════════════════════════════════════════
    # v7.4 GUARD 2 & 3: verify / verify_pro 스킵
    # ═══════════════════════════════════════════════════
    if not market_open:
        perf     = verify(today)
        pro_perf = verify_pro(today) if PRO_MODE else None
    else:
        print("[GUARD] verify / verify_pro 스킵 (오염 데이터 검증 방지)")
        perf     = {"win_rate": 0, "avg_return": 0, "top5_return": 0}
        pro_perf = None

    # ═══════════════════════════════════════════════════
    # v7.4 GUARD 4: save_signal_history 스킵
    # ═══════════════════════════════════════════════════
    if not market_open:
        save_signal_history(
            result["top20"], result["entry_top5"],
            regime, today, biz_day
        )
    else:
        print("[GUARD] save_signal_history 스킵 (히스토리 오염 방지)")
    ...
    final = {
        ...
        "mode"             : mode,
        "market_open"      : market_open,   # v7.4: 장중 실행 여부 표시
        "regime"           : regime,
    ...
    guard_str    = " [GUARDED]" if market_open else ""
    print(f"[DONE]{guard_str} {regime} | TOP20:{len(result['top20'])} | PRO:{len(pro_top)} | WIN:{perf['win_rate']}% | {pro_perf_str} | QUALITY:{data_quality}")
